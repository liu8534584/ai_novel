<template>
  <div class="prompt-page">
    <section class="prompt-hero glass-card">
      <div>
        <div class="pill pill-primary">提示词管理</div>
        <h1>统一管理与复用创作模板</h1>
        <p>沉淀高质量提示词，支持分类、版本与团队复用。</p>
      </div>
      <div class="hero-actions">
        <el-button type="primary" @click="openCreate">新建模板</el-button>
        <el-button @click="importDialog = true">导入模板</el-button>
      </div>
    </section>
    ...
    <section class="prompt-grid">
      <div class="glass-card prompt-card">
        <div class="card-header">
          <div>
            <h2>模板列表</h2>
            <p>按业务阶段管理系统、用户与复合提示词。</p>
          </div>
          <div class="header-actions">
            <el-input v-model="keyword" placeholder="搜索名称 / Key / 描述" clearable class="search-input" />
            <el-select v-model="category" placeholder="全部分类" clearable class="filter-select">
              <el-option label="全部分类" value="" />
              <el-option label="世界观" value="world" />
              <el-option label="大纲" value="plan" />
        ...
        <div v-if="filteredTemplates.length === 0" class="empty-area">
          <el-empty description="暂无模板，创建你的第一个提示词" />
        ...
        <div v-else class="card-grid">
          ...
          >
            ...
            <div class="item-footer">
              <div class="time">更新于 {{ item.updatedAt }}</div>
      ...
      <div class="glass-card prompt-side">
        <div class="card-header">
          <h2>推荐分组</h2>
          <p>按流程分组，快速定位可复用模板。</p>
        </div>
        <div class="group-list">
          <div v-for="group in groups" :key="group.name" class="group-item">
            <div>
              ...
              <div class="group-meta">{{ group.desc }}</div>
            </div>
        ...
        <div class="tip-card">
          ...
          <div class="tip-list">
            <div>保持目标清晰，避免一次性塞入过多要求。</div>
            <div>模板中拆分变量占位，便于动态替换。</div>
            <div>按“世界观-角色-章节”顺序沉淀模板体系。</div>
          </div>
        </div>
      </div>
    </section>

    <el-dialog v-model="createDialog" :title="editingId ? '编辑提示词模板' : '新建提示词模板'" width="720px">
      <el-form :model="createForm" label-width="90px">
        <el-form-item label="Key">
          <el-input v-model="createForm.key" placeholder="例如：planner_dark" />
        </el-form-item>
        <el-form-item label="名称">
          <el-input v-model="createForm.title" placeholder="例如：暗黑向大纲" />
        </el-form-item>
        ...
        <el-form-item label="描述">
          <el-input v-model="createForm.description" type="textarea" :rows="2" placeholder="模板用途或适用场景" />
        </el-form-item>
        <el-form-item label="模板内容">
          <el-input
            ...
            type="textarea"
            :rows="10"
            placeholder="输入模板正文，输入 {{ 触发变量提示"
            @input="syncVariableHint"
            @keyup="syncVariableHint"
            @click="syncVariableHint"
          />
          <div class="variable-available">
            <div class="variable-title">变量字典</div>
            <div class="variable-tags">
              <span v-for="item in allVariables" :key="item.key" class="variable-tag" @click="insertVariable(item)">
                {{ item.token }}
              </span>
            </div>
          </div>
          <div v-if="showVariablePicker" class="variable-panel">
            <div class="variable-title">输入 &#123;&#123; 后可选变量</div>
            <div class="variable-list">
              ...
              <div v-if="filteredVariables.length === 0" class="variable-empty">未匹配到变量</div>
            </div>
          </div>
        </el-form-item>
      </el-form>
    ...
    <el-dialog v-model="importDialog" title="导入提示词模板" width="520px">
      <el-form :model="importForm" label-width="90px">
        <el-form-item label="内容">
          <el-input v-model="importForm.content" type="textarea" :rows="6" placeholder="粘贴 JSON / Markdown 模板" />
        </el-form-item>
      </el-form>
      <template #footer>
...
`,
  ...
  {
    id: 8,
    key: 'chapter_title_plan',
    title: '章节标题（分阶段指令）',
    category: 'plan',
    description: '输出章节标题生成指令',
    content: `
你是一名网络小说章节策划总监。
...
`,
  ...
  {
    id: 9,
    key: 'chapter_title_batch',
    title: '章节标题（分批生成）',
    category: 'plan',
    description: '按批次生成后续章节标题',
    content: `
你是一名网络小说章节策划。
...
`,
  ...
  {
    id: 10,
    key: 'chapter_title_batch_plan',
    title: '章节标题（指令版分批）',
    category: 'plan',
    description: '基于标题指令分批生成章节标题',
    content: `
你是一名网络小说章节策划。
...
`,
  ...
  {
    id: 14,
    key: 'character_dynamic_state',
    title: '角色状态抽取',
    category: 'character',
    description: '提取本章角色状态变化',
    content: `
...
`,
  ...
  {
    id: 15,
    key: 'chapter_objective',
    title: '章节目标',
    category: 'plan',
    description: '为指定章节生成写作目标',
    content: `
...
`,
  ...
  },
  {
    ...
    title: '状态审计',
    category: 'review',
    description: '校对章节文本与状态 JSON 的变化',
    content: `
...
`,
  ...
  {
    id: 19,
    key: 'foreshadowing_resolution',
    title: '伏笔回收判断',
    category: 'review',
    description: '评估伏笔是否在本章回收',
    content: `
...
`,
  ...
  {
    id: 20,
    key: 'character_anchor_extraction',
    title: '性格锚点提取',
    category: 'character',
    description: '提取角色性格锚点与动机',
    content: `
...
重要：所有补全的内容都必须与对话中的信息保持高度关联，确保前后一致性。
...
const groupDefinitions = [
  { name: '世界观与设定', desc: '世界观与规则框架', categories: ['world'] },
  { name: '大纲与章节', desc: '剧情结构与章节规划', categories: ['plan'] },
  { name: '角色与状态', desc: '角色画像与状态追踪', categories: ['character'] },
  { name: '写作与正文', desc: '正文生成与写作策略', categories: ['writing'] },
  { name: '复盘与审计', desc: '一致性与质量评估', categories: ['review'] }
]
...
const groups = computed(() =>
  groupDefinitions.map((group) => ({
    name: group.name,
    desc: group.desc,
    count: templates.value.filter((item) => group.categories.includes(item.category)).length
  }))
)
...
const categoryLabel = (val: string) => {
  if (val === 'world') return '世界观'
  if (val === 'plan') return '大纲'
  if (val === 'character') return '角色'
  if (val === 'writing') return '写作'
  if (val === 'review') return '复盘'
  return '未分类'
}
...
const allVariables = computed(() => {
  const map = new Map<string, { key: string; label: string; token: string }>()
  templates.value.forEach((item) => {
    extractVariables(item.content).forEach((variable) => {
      const key = variable
      if (!map.has(key)) {
        map.set(key, {
          key,
          label: variable.replace(/([a-z])([A-Z])/g, '$1 $2').toLowerCase(),
          token: `{{.${variable}}}`
        })
      }
    })
...
const filteredVariables = computed(() => {
  const query = variableQuery.value.replace('.', '').trim().toLowerCase()
...
const saveTemplate = () => {
  if (!createForm.key || !createForm.title || !createForm.content) {
    ElMessage.error('请填写模板 Key、名称与内容')
    return
  }
  const existingIndex = templates.value.findIndex((item) => item.key === createForm.key)
  if (editingId.value === null && existingIndex >= 0) {
    ElMessage.error('Key 已存在，请更换')
    return
  }
  const now = new Date().toISOString().slice(0, 10)
  if (editingId.value !== null) {
    const index = templates.value.findIndex((item) => item.id === editingId.value)
    const current = index >= 0 ? templates.value[index] : null
    if (current) {
      templates.value[index] = {
        ...current,
        key: createForm.key,
        title: createForm.title,
        category: createForm.category || 'writing',
    ...
    const newItem: PromptTemplate = {
      id: Math.max(...templates.value.map((item) => item.id)) + 1,
      key: createForm.key,
      title: createForm.title,
      category: createForm.category || 'writing',
      description: createForm.description,
    ...
    templates.value.unshift(newItem)
  }
  createDialog.value = false
  ElMessage.success(editingId.value ? '模板已更新' : '模板已添加到列表')
}

const handleImport = () => {
  if (!importForm.content) {
    ElMessage.error('请粘贴模板内容')
    return
  }
...
const syncVariableHint = () => {
  ...
  if (openIndex > closeIndex) {
    ...
    variableQuery.value = beforeCursor.slice(openIndex + 2)
  } else {
    showVariablePicker.value = false
    variableQuery.value = ''
  }
...
const insertVariable = (item: { key: string; token: string }) => {
  const textarea = contentInput.value?.textarea || contentInput.value?.$el?.querySelector('textarea')
  if (!textarea) {
    createForm.content += item.token
    return
  }
  const cursor = textarea.selectionStart || 0
  const content = createForm.content
  ...
  const insertStart = openIndex > closeIndex ? openIndex : cursor
  const before = content.slice(0, insertStart)
  const after = content.slice(cursor)
  createForm.content = `${before}${item.token}${after}`
  showVariablePicker.value = false
  variableQuery.value = ''
  nextTick(() => {
    const nextPos = before.length + item.token.length
